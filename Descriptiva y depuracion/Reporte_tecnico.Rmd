...
Para estos modelos se decidio ajustar una Regresión Poisson y una regresión Binomial negativa. Se considera que son óptimas porque el número de accidentes es un conteo.
...
__Modelo 1:__
```{r}
kable(com.mes.1[,c(1,4,5)])%>%
  kable_styling("striped", full_width = F,fixed_thead = T)
```

__Modelo 2:__
```{r}
kable(com.mes.2[,c(1,4,5)])%>%
  kable_styling("striped", full_width = F,fixed_thead = T)
```
...
En general los barrios con una cantidad pequeña de accidentes ajustan modelos deficientes. Las tablas resumen se presentan solo para los 18 barrios con mayor número de accidentes.
...
__Modelo 1:__
```{r}
kable(bar.dia.1[bar.dia.1$Barrio %in% unlist(barrios),c(1,4)])%>%
  kable_styling("striped", full_width = F,fixed_thead = T)
```

__Modelo 2:__
```{r}
kable(bar.dia.2[bar.dia.2$Barrio %in% unlist(barrios),c(1,4)])%>%
  kable_styling("striped", full_width = F,fixed_thead = T)
```

__Tabla comparativa de los MSE__
```{r}
temp <- cbind(bar.dia.1[bar.dia.1$Barrio %in% unlist(barrios),1:3], bar.dia.2[bar.dia.2$Barrio %in% unlist(barrios),2:3])
...
Para los 18 barrios con más datos la suma de los MSE fueron:
- Modelo 1 fue de 35.68 para entrenamiento y 37.28 para validación, 
- Modelo 2 fueron 36.08 y 41.32.
Sin embargo en general la suma de los MSE de entrenamiento son para entrenamiento y validación respectivamente:
- Modelo 1: 124.2207 y 116.711.
- Modelo 2: 261.9699 y 133.3192.
...
__Modelo 1:__
```{r}
kable(bar.sem.1[bar.sem.1$Barrio %in% unlist(barrios),c(1,3)])%>%
  kable_styling("striped", full_width = F,fixed_thead = T)
```

__Modelo 2:__
```{r}
kable(bar.sem.2[bar.sem.2$Barrio %in% unlist(barrios),c(1,3)])%>%
  kable_styling("striped", full_width = F,fixed_thead = T)
```

__Tabla comparativa de los MSE__
```{r}
temp <- cbind(bar.sem.1[bar.sem.1$Barrio %in% unlist(barrios),1:3], bar.sem.2[bar.sem.2$Barrio %in% unlist(barrios),2:3])
colnames(temp) <- c("BARRIO", "MSE.tr Modelo 1", "MSE.va Modelo 1", "MSE.tr Modelo 2", "MSE.va Modelo 2" )
kable(temp)%>%
  kable_styling("striped", full_width = F,fixed_thead = T)
```
En este caso se encuentra que la suma de los MSE de entrenamiento y validación son respectivamente para los 18 barrios con mayor accidentalidad son:
- Modelo 1: 353.4326,334.726. 
- Modelo 2: 350.6062, 354.8639.
Usando todos los barrios son mayor accidentalidad son:
- Modelo 1: 939.4501, 910.3487. 
- Modelo 2: 793.9533, 856.0634.
...
formula("total~I(PERIODO^2)+MES+PERIODO"),
            formula("total~MES"),
...
__Modelo 1:__
```{r}
kable(bar.mes.1[bar.mes.1$Barrio %in% unlist(barrios),-5])%>%
  kable_styling("striped", full_width = F,fixed_thead = T)
```

__Modelo 2:__
```{r}
kable(bar.mes.2[bar.mes.2$Barrio %in% unlist(barrios),-5])%>%
  kable_styling("striped", full_width = F,fixed_thead = T)
```

__Tabla comparativa de los MSE__
```{r}
temp <- cbind(bar.mes.1[bar.mes.1$Barrio %in% unlist(barrios),1:3], bar.mes.2[bar.mes.2$Barrio %in% unlist(barrios),2:3])
colnames(temp) <- c("BARRIO", "MSE.tr Modelo 1", "MSE.va Modelo 1","MSE.tr Modelo 2", "MSE.va Modelo 2" )
kable(temp)%>%
  kable_styling("striped", full_width = F,fixed_thead = T)
```
