...
date_filler <- function(df, barrio){
  ...
  dates <- data.frame(FECHA=seq(as.Date('2014-01-01'),as.Date('2018-01-31'),by = "week"))
  dates <- dates %>% mutate(
...
#test <- filter(barrios_sem, PERIODO!=2018, BARRIO == "La Candelaria")
#test2 <- date_filler(test, "La Candelaria")
```
...
adj_model_com <- function(formulas, tr_data, val_data,zero_ex_cond=F){
  ...
  for(co in com){
    ...
    for(formula in formulas){
      ...
      list_models = list(model1)
      
      
       
       mse_tr.v  <- c(mse_tr.v, mean((temp_tr$total - predict(model1, newdata=temp_tr))^2))
      mse_val.v <- c(mse_val.v, mean((temp_va$total - predict(model1,newdata= temp_va))^2))
...
formulas = c(formula("total~SEMANA+I(PERIODO^2)+MES+PERIODO"),
            ...
            ,formula("total~SEMANA+I(PERIODO^2)+PERIODO+MES+I(MES^2)")
...
```{r}
TR_data <- filter(barrios_mes, PERIODO!=2018)
VA_data <- filter(barrios_mes, PERIODO==2018)
...
bar.mes.1.vis <- bar.mes.1[,-5]


```

```{r}
TR_data <- filter(barrios_mes, PERIODO!=2018 & PERIODO!=2017 & BARRIO!="San José la Cima No.2")
VA_data <- filter(barrios_mes, (PERIODO==2018 | PERIODO==2017)&BARRIO!="San José la Cima No.2")
bar.mes.2 <- adj_model_com(formulas, TR_data, VA_data)
bar.mes.2.vis <- bar.mes.2[,-5]
```


```{r}
#MSE_mes_bar
save(bar.mes.1, bar.mes.2, file="./Modelos_finales/bar.mes.1_2.RData")
```
...
formulas = c(formula("total~I(PERIODO^2)+MES+PERIODO+SEMANA"),
            formula("total~MES+ SEMANA"),
            formula("total~MES"),
            formula("total~MES+PERIODO+SEMANA")
)

bar.sem.1 <- adj_model_com(formulas, TR_data, VA_data,T)
bar.sem.1.vis <- bar.sem.1[,-5]
...
bar.sem.2 <- adj_model_com(formulas, TR_data, VA_data,T)
bar.sem.2.vis <- bar.sem.2[,-5]
```

```{r}
#MSE_sem_bar
save(bar.sem.1, bar.sem.2, file="./Modelos_finales/bar.sem.1_2.RData")
```
