...
## Transformación mahalanobis
```{r}
C <- chol( var(cluster_df) )
C <-solve(C)
y <- as.matrix(cluster_df) %*% C
cluster_maha<- (kmeans(y, 4))
```
...
cluster7 <- kmeans(cluster_df, centers = 7, iter.max=1000, nstart=100)
