...
predicciones_semana <- function(com.or.bar, inicio, final, nombre){
  ...
  if(com.or.bar){
    ...
    y2 <- predict(modelo2, newdata = datos[,-5], data= tr2)
  }else{
    #TODO VERIFICAR
  ...
  p1 <- ggplot(data = df1, aes(y= y1, x = as.Date(Fecha), col=as.factor(PERIODO)))+ geom_point()+
     geom_line()+
  ...
  p2 <- ggplot(data = df2, aes(y= y2, x = as.Date(Fecha), col=as.factor(PERIODO)))+ geom_point()+
     geom_line()+
  theme(legend.position="top")+
  theme_bw()+
    scale_x_date(date_labels = "%U")+
  ...
  output <- list(df1, round(total1), p1, df2, round(total2), p2)
