...
predicciones_meses <- function(com.or.bar, inicio, final, nombre){
  ...
  if(com.or.bar){
    ...
    y1 <- predict(modelo1, newdata = datos[,-4], data= tr1)
    y2 <- predict(modelo2, newdata = datos[,-4], data= tr2)
    ...
    y1 <- predict(modelo1, newdata = datos[,-4])
  ...
  labs(title=paste("Accidentalidad en", nombre),
        x ="Mes", y = "Número de accidentes", col="Periodo")
...
prediccion <- predicciones_meses(T, c(2018, 5), c(2019,10), "La Candelaria")
