...
date_filler <- function(df, barrio){
  ...
  df <- df[,-6]
  dates <- data.frame(FECHA=seq(as.Date('2014-01-01'),as.Date('2017-01-31'),by = 1))
...
adj_model_com <- function(formulas, tr_data, val_data, zero_ex_cond = F){
  ...
  for(co in com){
    ...
    if(zero_ex_cond){
      ...
      tr_data <- filter(temp, PERIODO!=2018 & PERIODO!=2017)
      val_data <- filter(temp, PERIODO==2018 | PERIODO!=2017)
    }
    ...
    for(formula in formulas){
      ...
      model1 <- glm(formula = formula, data = temp_tr)
      ...
      list_models = list(model1)#QUite model3
       ...
       mse_tr.v  <- c(mse_tr.v, mean((temp_tr$total - predict(model1, newdata=temp_tr))^2))
       
      mse_val.v <- c(mse_val.v, mean((temp_va$total - predict(model1,newdata=temp_tr))^2))
      ...
      best_model$form <- format(formula(best_model$model[[1]]))
  ...
  colnames(results) <- c("MSE.tr", "MSE.va","Reg.obj", ub_name, "Formula")
  results <- results[,c(4,1,2,5,3)]
  return(results)
...
bar.dia.1.vis <- bar.dia.1[,-5]
```

```{r}
entrenamiento.real <- adj_model_com(formulas, TR_data, VA_data, T)
entrenamiento.dup <- date_filler(temp_tr, "Aldea Pablo VI")
s <- bar.dia.1[1,6][[1]]
temp_tr <- filter(df.barrio, PERIODO!=2018, BARRIO =="Aldea Pablo VI")
predict(s, newdata = temp_tr[,-6], data = entrenamiento.dup)
```
