...
carga_bd <- function(){
  ...
  #Persona
  caracteristicas_y_composicion_hogar <- read.table(file = "txt2016/Características y composición del hogar.txt", header = T, sep = "\t") %>%
    rename(edad = P6040) %>%
    select(DIRECTORIO,SECUENCIA_ENCUESTA ,SECUENCIA_P, edad)
  
  salud <- read.table(file = "txt2016/Salud.txt", header = T, sep = "\t") %>%
  ...
  educacion <- read.table(file = "txt2016/Educación.txt", header = T, sep = "\t") %>%
    rename( nivel_educativo = P8587 ) %>%
    select(DIRECTORIO,SECUENCIA_ENCUESTA ,SECUENCIA_P, nivel_educativo)
  
  fuerza_de_trabajo <- read.table(file = "txt2016/Fuerza de trabajo.txt", header = T, sep = "\t") %>%
  ...
  #Hogar
  servicios_del_hogar <- read.table(file = "txt2016/Servicios del hogar.txt", header = T, sep = "\t") %>%
  ...
  tenencia_y_financiacion_vivienda <- read.table(file = "txt2016/Tenencia y financiación de la vivienda que ocupa el hogar.txt", header = T, sep = "\t")
  ...
  condiciones_de_vida <- read.table(file = "txt2016/Condiciones de vida del hogar y tenencia de bienes.txt", header = T, sep = "\t") 
  ...
  #vivienda
  viviendas <- read.table(file = "txt2016/Datos de la vivienda.txt", header = T, sep = "\t") %>%
...
carga_personas <- function(){
  ...
  bd_familias <- read.table(file = "txt2016/Características y composición del hogar.txt", header = T, sep = "\t")
...
grafica_hogar_de_persona <- function(idhogar){
  ...
  res <- procesar_hogar(hogares2016[[idhogar]])
...
hogares2016 <- procesar_todos_hogares()
...
MS_2016 <- MS_filter %>% select(edad, regimen, estado_de_salud,
                           nivel_educativo, actividad_semana_pasada,
                           arriendo, vivienda_propia, seguridad,
                           bienes, estrato, ingreso_percapita, personas_hogar)
MS_2016 <- na.omit(MS_2016)

satisfaccion_2016 <- data.frame(satisfaccion=(10-predict(MG1, newdata = MS_2016, type="response")))
satisfaccion_2016 <- round(satisfaccion_2016)
MS_2016 <- cbind(satisfaccion_2016, MS_2016)
...
write.table(bd_estudio, "txt2016/bd_estudio.txt", sep="\t")
write.table(bd_familias, "txt2016/bd_familias.txt", sep="\t")
write.table(MS_WEB, "txt2016/madres_solteras.txt", sep="\t")
MS_WEB_REAL <- read.table(file="txt2016/madres_solteras.txt", sep="\t")
save(hogares2016, file="txt2016/hogares2016.RData")
